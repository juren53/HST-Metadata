...
# Updated Mon 01 Jul 2024 01:09:29 PM CDT  Renamed to audio-tags.py & csv_filename variable created ver
# ----------------------------------------------------------------
...
with open(csv_filename, 'r') as csvfile:
    ...
    for row in reader:
        ...
        # Open a temporary file to discard stdout and stderr
        temp_file = open('temp.txt', 'w') # Use 'w' mode to truncate the file each time


        # Run the FFmpeg command that creates the custom thumbnail
        subprocess.run(cmd, stdout=temp_file, stderr=temp_file)
        ...
        thumbnail_command = [
            ...
            '-id3v2_version', '3',
            #'-colorspace YUV',          ####################
            '-metadata:s:v', 'title="HST"',
            '-metadata:s:v', 'comment="HST"',
            output_file
