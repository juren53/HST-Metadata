...
def convert_date(date_str):
    ...
    elif re.match(r'[A-Za-z]+, \d{2}/\d{2}/\d{4}', date_str):   # Monday, 03/23/1964
        try:
    ...
    if re.match(r'[A-Za-z]+ \d{4}', date_str):  # convert Month YYYY to YYYY-DD-00 eg. July 1948
        try:
            date_object = datetime.strptime(date_str, "%B %Y")
    ...
    if re.match(r'[A-Za-z]+, \d{4}', date_str):  #  July, 1948
        try:
            date_object = datetime.strptime(date_str, "%B, %Y")
    ...
    if re.match(r"([A-Za-z]+) (\d{1,2}), (\d{4})", date_str):  # September 18, 1945
        try:
