...
To begin, it is important to remember 'not all Google Documents are created equal' - - while they may **appear** the same in a web browser and the URLs for the various Google Docuemtns appear the same, there a subtle but improtant differences.  To access the data programatically [i.e. via Python, etc.], the Google Document must be in a *native* Google format.  Such is the case with Google Sheets and Google Spreadsheets.  Google Sheets are a native Google format that can be accessed via the Google APIs and Google Spreadsheets are Excel XLSX files uploaded to Google's cloud service and look exactly like a Google Sheet in a Web browser but can not be accessed by Google's APIs.

You can determine whether a Google URL points to a native Google Sheet or an uploaded Excel file by using the Google Drive API. The key is to check the file’s mimeType. Native Google Sheets use the mimeType application/vnd.google-apps.spreadsheet, while uploaded Excel files retain their original format—for example, .xlsx files have the mimeType application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, even when viewed in Google Sheets.
