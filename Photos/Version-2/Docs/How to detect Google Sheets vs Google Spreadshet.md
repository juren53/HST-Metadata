...
To begin, it is important to remember 'not all Google Documents are created equal' - - while they may appear the same when pulled up in a web browser, if you want to access the data programatically [i.e. via Python, etc.], some work with Google's APIs and some don't.  Such is the case with Google Sheets and Google Spreadsheets.  Google Sheets are a native Google format that can be accessed via the Google APIs and Google Spreadsheets are Excel XLSX files uploaded to Google's cloud service and look exactly like a Google Sheet in a Web browser but can not be accessed by Google's APIs.
