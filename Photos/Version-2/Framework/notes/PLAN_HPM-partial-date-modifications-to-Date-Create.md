...
# Extract and validate each component independently
year_int = int(year_str) if year_str and year_str.isdigit() else 0
month_int = int(month_str) if month_str and month_str.isdigit() else 0  
day_int = int(day_str) if day_str and day_str.isdigit() else 0

# Apply range validation only to present components
if year_int and year_int <= 0:
    return ""  # Invalid year
if month_int and not (1 <= month_int <= 12):
    return ""  # Invalid month
if day_int and not (1 <= day_int <= 31):
    return ""  # Invalid day

# Always format with zero-padded components
return f"{year_int:04d}-{month_int:02d}-{day_int:02d}"
```
...
- **Range validation:** Only applied to present components
...
5. **All components blank** → Empty string
...
**Next Step:** Implement the new `get_date_from_columns` function logic