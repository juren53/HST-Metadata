...
Ver 0.5.0  - added support for g2c-generated CSV files and command line arguments
"""
...
def find_objectname_column(header_line, g2c_mode=False):
    """Find the column index for ObjectName in the header
    
    Args:
        header_line: The CSV header line
        g2c_mode: If True, look for g2c-specific column names
    """
    try:
        ...
        if g2c_mode:
            # For g2c-generated files, look for common g2c IPTC field names
            g2c_objectname_variants = ['objectname', 'headline', 'caption-abstract']
            
            # First try to find ObjectName specifically
            for i, header in enumerate(headers):
                if header.lower().strip() == 'objectname':
                    return i
            
            # If not found, try other g2c fields that might serve as identifiers
            for variant in g2c_objectname_variants[1:]:  # Skip 'objectname' as we already tried it
                for i, header in enumerate(headers):
                    if variant in header.lower().strip():
                        return i
        else:
            # Standard mode - look for ObjectName column (case insensitive)
            for i, header in enumerate(headers):
                if header.lower().strip() == 'objectname':
                    return i
...
def _print_report_header(filename, total_special, g2c_mode=False):
    out(f"Analyze UTF-8 Report - Version {VERSION}")
    if g2c_mode:
        out("Mode: G2C-generated CSV analysis")
    out("=" * 50)
    out(f"Analyzing file: {filename}")
    out(f"Total 'special characters' found: {total_special}")
    out()

def _print_report_footer(filename, total_special, g2c_mode=False):
    out()
    out("=" * 50)
    out("SUMMARY")
    out("=" * 50)
...
def analyze_csv_file(filename, g2c_mode=False):
    """Analyze CSV file for encoding issues
    
    Args:
        filename: Path to the CSV file to analyze
        g2c_mode: If True, optimize for g2c-generated CSV files
    """
    issues_found = False
    ...
    try:
        ...
        if lines:
            ...
            objectname_column_index = find_objectname_column(header_line, g2c_mode)
        
        # Collect issues and totals before printing header
        issues, total_special = _collect_issues(lines, objectname_column_index)
        _print_report_header(filename, total_special, g2c_mode)
        out(f"Successfully read {len(lines)} lines as UTF-8")
        if objectname_column_index is not None:
            # Get the actual column name from the header
            csv_reader = csv.reader([header_line])
            headers = next(csv_reader)
            column_name = headers[objectname_column_index]
            out(f"Found identifier column '{column_name}' at index {objectname_column_index}")
        else:
            if g2c_mode:
                out("No suitable identifier column found in g2c CSV - will show 'N/A' for ObjectName")
            else:
        ...
        _print_report_footer(filename, total_special, g2c_mode)
    ...
    try:
        ...
        # Extract the base filename without extension for the report name
        csv_basename = os.path.splitext(os.path.basename(filename))[0]
        
        # Create report filename with CSV name included
        report_filename = f"REPORT_UTF8-{csv_basename}-{ts}.txt"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Analyze CSV file for non-UTF-8 characters and report their locations',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Analyze default export.csv file
  python3 analyze_utf8.py
  
  # Analyze specific CSV file
  python3 analyze_utf8.py myfile.csv
  
  # Analyze g2c-generated CSV file with optimized column detection
  python3 analyze_utf8.py --g2c export.csv
  python3 analyze_utf8.py -g myfile.csv

Note: Reports are automatically saved as timestamped REPORT_UTF8-csvname-YYYY-MM-DD_HH-MM-SS.txt files.
"""
    )
    ...
    parser.add_argument(
        ...
        nargs='?',
        default='export.csv',
        help='CSV file to analyze (default: export.csv)'
    )
    
    parser.add_argument(
        '--g2c', '-g',
        action='store_true',
        help='Optimize analysis for g2c-generated CSV files (looks for IPTC field names)'
    )
    
    parser.add_argument(
        '--version', '-v',
        action='version',
        version=f'analyze_utf8.py version {VERSION}'
    )
    ...
    # Show mode info
    if args.g2c:
        out(f"🔍 G2C Mode: Analyzing g2c-generated CSV file: {args.filename}")
        out("   Looking for IPTC field names (ObjectName, Headline, Caption-Abstract)")
        out()
    else:
        out(f"📊 Standard Mode: Analyzing CSV file: {args.filename}")
        out()
    
    analyze_csv_file(args.filename, g2c_mode=args.g2c)
