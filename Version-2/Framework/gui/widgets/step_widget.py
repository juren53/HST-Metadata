...
class StepWidget(QWidget):
    ...
    def _init_ui(self):
        ...
        version_label = QLabel(
            "<span style='font-size: 10pt;'>v0.1.7b | 2026-01-17 16:05 CST</span>"
        )
    ...
    def _review_step(self, step_num: int):
        ...
        step_config = self.framework.config_manager.get(
            f"step_configurations.step{step_num}", {}
        )
        completed = self.framework.config_manager.get_step_status(step_num)
        ...
        review_text += (
            f"<p><b>Status:</b> {'✅ Completed' if completed else '⭕ Pending'}</p>"
        )
        ...
        if step_num == 1:
            ...
            review_text += (
                f"<p><b>Encoding:</b> {step_config.get('encoding', 'utf-8')}</p>"
            )
        elif step_num == 4:
    ...
    def _review_step_7_open_directory(self):
        ...
        try:
            ...
            if QDesktopServices.openUrl(url):
                self.output_text.append(
                    f"✓ Opened resized JPEG directory: {resized_jpeg_dir}\n"
                )
            else:
                QMessageBox.warning(
                    self,
    ...
    def _review_step_8_open_directory(self):
        ...
        try:
            ...
            if QDesktopServices.openUrl(url):
                self.output_text.append(
                    f"✓ Opened watermarked JPEG directory: {watermarked_jpeg_dir}\n"
                )
            else:
                QMessageBox.warning(
                    self,
    ...
    def _revert_step(self, step_num: int):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            if step_num == 2:
                ...
                if data_directory:
                    tiff_processed_dir = (
                        Path(data_directory) / "output" / "tiff_processed"
                    )
                ...
                if data_directory:
                    watermarked_jpeg_dir = (
                        Path(data_directory) / "output" / "jpeg_watermarked"
                    )
    ...
    def _validate_all(self):
        ...
        if success:
            ...
            QMessageBox.information(
                self, "Validation", "All validations passed successfully"
            )
        else:
            self.output_text.append("⚠️ Validation found issues\n")
            QMessageBox.warning(
                self, "Validation", "Validation found some issues. Check the output."
            )
