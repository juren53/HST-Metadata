...
# This script processes Excel files by:
# 1. Allowing user to select an Excel file from the current directory
...
def get_excel_files():
    ...
    return [file for file in os.listdir() if file.endswith(('.xlsx', '.xls'))]

def select_excel_file():
    """Let user select an Excel file from the available files."""
    # Clear the screen
    ...
    while True:
        try:
            ...
            if 0 <= selected_file_index < len(excel_files):
                return excel_files[selected_file_index]
            else:
...
def convert_excel_to_csv(input_file=None):
