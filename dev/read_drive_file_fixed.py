...
def get_valid_file_id():
    ...
    print("Example Sheets URL: https://docs.google.com/spreadsheets/d/1a2b3c4d5e6f7g8h9i0j/edit")
...
def get_file_metadata(service, file_id):
    ...
    """
    try:
        # Try both with and without shared drive support
        try:
            ...
            # If that fails, try without supportsAllDrives
            if "supportsAllDrives" in str(e):
                return service.files().get(
                    fileId=file_id, 
                    fields="id, name, mimeType, size, modifiedTime"
                ).execute()
        ...
        if "not found" in str(error).lower():
            ...
            print("2. The file might be in a shared drive that you don't have access to")
            print("3. You don't have permission to access this file")
            print("4. The file might have been deleted or moved")
            
            # Try to search for similar files to help user
            try:
                results = service.files().list(
                    q=f"name contains '{file_id}' or '{file_id}' in parents",
                    pageSize=5,
                ...
                if items:
                    print("\nFound some possibly related files:")
                    for item in items:
            ...
            print("1. Have at least 'Reader' access to this file")
            print("2. Are using the correct Google account")
            print("3. Have accepted any sharing invitations for this file")
            
        return None
    except Exception as error:
        print(f"Unexpected error retrieving file metadata: {error}")
        return None

def list_accessible_files(service, max_results=20):
    """List files that the user has access to."""
    print("\nListing the most recent files you have access to:")
    
    try:
        # First try with supportsAllDrives
        try:
            results = service.files().list(
        ...
        print("\nRecent files you can access:")
        for item in items:
            mod_time = item.get('modifiedTime', 'unknown date')
            if 'T' in mod_time:
                mod_time = mod_time.split('T')[0]  # Just show the date part
                
            print(f"- {item.get('name')} (ID: {item.get('id')}) [{mod_time}]")
            
        print("\nYou can try one of these file IDs.")
    ...
    try:
        ...
        if mime_type.startswith('application/vnd.google-apps'):
            ...
            try:
                ...
                if "supportsAllDrives" in str(e):
                    # Fall back without supportsAllDrives
